...
def update_ckpt(args):
    basic_dir = "tmp/{}/{}/{}".format(args.model, args.ckpt.replace(".pth", ""), datetime.datetime.now().strftime('%y_%m_%d_%H_%M_%S.%f')[:-4])
    if not args.debug:
        config = vars(args)
...
def get_average_gap_search(metric, dataset_name, forget_percent):
    d = {
        "cifar10": (93.34, 99.94, 94.08, 83.88),
        "cifar100": (76.68,99.94,77.60,55.40),
        "tinyimagenet": (66.55,99.97,67.06,44.54),
    }
    avg_gap = 0
    avg_gap -= metric[0] - d[dataset_name][0]
    avg_gap -= metric[1] - 99.94
    avg_gap += abs(metric[0]-metric[2])
    avg_gap += abs(metric[2] -d[dataset_name][2]) * 0.5
    return avg_gap/3

    

def get_average_gap(metric, dataset_name, forget_percent, weight=[1,1,1,1]):
    
    d = {
        "cifar10-1": (93.34, 99.94, 94.08, 83.88),
        "cifar10-10": (93.12,99.94,93.20,80.80),
        "cifar100-1": (76.68,99.94,77.60,55.40),
        "cifar100-10": (75.43,99.95,75.92,53.49),
        "tinyimagenet-1": (66.55,99.97,67.06,44.54),
        "tinyimagenet-10": (65.45,99.97,65.29,41.94),
    }
    metric_retrain = d[f"{dataset_name}-{forget_percent}"]
    avg_gap = 0
    avg_gap += abs(metric[0] - metric_retrain[0]) * weight[0]
    avg_gap += abs(metric[1] - metric_retrain[1]) * weight[1]
    avg_gap += abs(metric[2] - metric_retrain[2]) * weight[2]
    avg_gap += abs(metric[3] - metric_retrain[3]) * weight[3]
    return avg_gap/4
