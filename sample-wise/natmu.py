...
def get_dataloader(args, num_of_classes, net,device):
    ...
    record = []
    num_of_samples_in_retain = len(retainDS_inTrainSet_woDataAugment) / num_of_classes
    tmp_num = int(num_of_samples_in_retain * 0.98)
    for i in range(num_of_classes):
        if (random_label==i).sum() < num_of_samples_in_retain * 0.4:
            record.append(i)
        if (random_label==i).sum() > tmp_num:
            np.random.shuffle(record)
            j = record.pop()
            new_label = np.array([i for _ in range(tmp_num)] + [j for _ in range((random_label==i).sum() -tmp_num)])
            random_label[random_label==i] = new_label
